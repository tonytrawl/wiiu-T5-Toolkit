...
OUT = os.path.join(_ROOT, 'dlc loading', 'native', 'fullrelink', '_zm_reemit_nogrow')
...
def main():
    ...
    # NO GROWTH: re-emit the SAME console table (4 maps) unchanged. Isolates whether
    # emit_stringtable's reconstruction (all-FOLLOW cells + cellIndex resort) alone
    # breaks UI_LoadMaps, independent of any added rows.
    merged = [list(r) for r in con_table]
    print('re-emitting console table UNCHANGED: %d rows, %d map rows'
          % (len(merged), sum(1 for r in merged if r and r[0].startswith('zm_'))))
