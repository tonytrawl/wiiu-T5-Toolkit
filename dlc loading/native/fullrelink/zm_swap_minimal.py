...
OUT = os.path.join(_ROOT, 'dlc loading', 'native', 'fullrelink', '_zm_swap_minimal')
...
def main():
    ...
    for i, rr in enumerate(merged):
        if rr and rr[0] == SWAP_OUT:
            newrow = list(rr)                              # KEEP transit_dr's entire row
            newrow[0] = SWAP_IN                            # change ONLY internal name
            merged[i] = newrow
            print('swapped slot %d: %s -> %s (map index kept=%s, pack=0)'
                  % (i, SWAP_OUT, SWAP_IN, newrow[5] if 5<len(newrow) else ''))
