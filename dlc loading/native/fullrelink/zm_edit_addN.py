...
PACK_COL = 11                   # DLC map-pack index column
PACK_VAL = '0'                  # force new rows to base-game pack (un-gated, in-range)
...
OUT = os.path.join(_ROOT, 'dlc loading', 'native', 'fullrelink', '_zm_addN')
ADD_N = 1   # how many new zm maps to add
...
def main():
    ...
    # ONLY change vs the plain additive edit: force new rows' DLC pack index in-range.
    newnames = {row[0] for row in new_rows}
    patched = 0
    for row in merged:
        if row and row[0] in newnames:
            old = row[PACK_COL] if PACK_COL < len(row) else ''
            while len(row) <= PACK_COL:
                row.append('')
            row[PACK_COL] = PACK_VAL
            patched += 1
            print('   %-12s col%d pack %s -> %s (display kept real)' % (row[0], PACK_COL, old, PACK_VAL))
    print('added %d new zm rows with pack=%s: %s' % (patched, PACK_VAL, ', '.join(sorted(newnames))))

    # keep only the first ADD_N new maps (empirical ceiling search)
    keep=[]
    seen_new=0
    for row in merged:
        if row and row[0] in newnames:
    ...
    for row in merged:
        if row and row[0]=='maxnum_map':
            nmaps=sum(1 for r in merged if r and r[0].startswith('zm_'))
            row[1]=str(nmaps)
            print('   trimmed to ADD_N=%d -> maxnum_map=%d, total map rows=%d'%(ADD_N,nmaps,nmaps))
