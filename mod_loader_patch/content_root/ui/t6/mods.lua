-- T6.Mods — Wii U first-run adaptation.
-- Hardened vs. the PC Plutonium mods.lua so it CANNOT crash on the Wii U build:
--   * CoD.BOIIOrange (a Plutonium-added colour) -> local fallback.
--   * Engine.GetModCount / Engine.GetModInfo / "loadmod" may NOT exist on the
--     stock Wii U engine yet (those are the RPL bindings still to be written).
--     Every call is guarded; if the bindings are absent the menu still opens and
--     shows a friendly "loader not installed" line instead of erroring.
-- Goal of this file for the first run: prove a brand-new .lua (one that exists
-- ONLY on disk, never in the fastfile) loads via the RPL raw-file hook.
...
-- Fallback colour if the Plutonium orange is not defined on this build.
local TitleColor = CoD.BOIIOrange or { r = 1, g = 0.5, b = 0 }

-- Safe wrappers around the (possibly-absent) engine bindings.
local function SafeGetModCount()
	if Engine and Engine.GetModCount then
		local ok, n = pcall( Engine.GetModCount )
		if ok and type( n ) == "number" then return n end
	end
	return 0
end

local function SafeGetModInfo( i )
	if Engine and Engine.GetModInfo then
		local ok, info = pcall( Engine.GetModInfo, i )
		if ok then return info end
	end
	return nil
end

local function BindingsPresent()
	return Engine ~= nil and Engine.GetModCount ~= nil
end
...
CoD.ModInfo.new = function()
	...
	title:setRGB( TitleColor.r, TitleColor.g, TitleColor.b )
...
local function LoadMod( self, event )
	if CurrentModInfo and CurrentModInfo.modname ~= nil then
		-- "loadmod" may not exist yet on Wii U; harmless if the command is unknown.
...
function LUI.createMenu.LoadModModal( controller )
	...
	self.buttonList = CoD.ButtonList.new( {
		leftAnchor = true, rightAnchor = false, left = 0, right = CoD.ButtonList.DefaultWidth,
		topAnchor = true, bottomAnchor = true, top = CoD.textSize.Big + 10, bottom = 0
	} )
...
local function refreshModList( self )
	for k in pairs( ModsInfo ) do ModsInfo[k] = nil end
	local modCount = SafeGetModCount()
	for i = 0, modCount - 1, 1 do
		local modInfo = SafeGetModInfo( i )
		if not modInfo then break end
...
local function modListUnloadEventHandler( self, event )
	if Dvar and Dvar.fs_game and Dvar.fs_game:get() ~= "" then
...
local function modListFocusChangedEventHandler( self, event )
	...
	if modInfo ~= nil then
		...
		self.modInfo.author:setText( "Author: " .. tostring( modInfo.author ) )
		self.modInfo.description:setText( "Description: " .. tostring( modInfo.description ) )
		self.modInfo.version:setText( "Version: " .. tostring( modInfo.version ) )
	end
end
...
function LUI.createMenu.Mods( controller )
	...
	if BindingsPresent() then
		listBox.noDataText = "No mods found for the current mode.\nMake sure mod is prefixed with \"" .. modPrefix .. "\" to be found."
	else
		listBox.noDataText = "Mod loader bindings not installed on this build yet.\n(This menu loaded from disk = the raw-file hook works!)"
	end
