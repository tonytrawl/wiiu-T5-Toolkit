...
pcall(require, "T6.Mods")  -- loose-file safe: missing mods.lua must not brick the lobby
if CoD.isWIIU then
...
CoD.MainLobby.OpenModsList = function(MainLobbyWidget, ClientInstance)
	if LUI.createMenu.Mods == nil then
		-- mods.lua did not load (hook inactive or file missing); do nothing rather than error
		return
	end
